...
class NavimowPathImage(ImageEntity):
    ...
    def __init__(self, hass, coordinator, device: dict, mqtt: NavimowMQTTHandler) -> None:
        ...
        device_name: str = device.get("deviceName", "Navimow")
    ...
    def _render(self) -> bytes | None:
        ...
        if not _PIL:
            _LOGGER.warning("Pillow er ikke installeret — kan ikke tegne klippesti")
            return None
        if not points:
            return self._placeholder()
        return self._draw_path(points)

    def _draw_path(self, points: list[tuple[float, float]]) -> bytes:
        xs, ys = zip(*points)
        ...
        def px(x: float, y: float) -> tuple[int, int]:
            return (
                int(_PAD + (x - min_x) * scale),
                int(_SIZE - _PAD - (y - min_y) * scale),
            )
