...
class NavimowMQTTHandler:
    ...
    async def _run(self) -> None:
        import aiomqtt
        topics = [
            f"/downlink/vehicle/{dev}/realtimeDate/location" for dev in self._device_ids
        ] + [
            f"/downlink/vehicle/{dev}/realtimeDate/state" for dev in self._device_ids
        ]
    ...
    def _handle_location(self, device_id: str, data: dict) -> None:
        ...
        if pos_x is None or pos_y is None:
            return
        self._paths.setdefault(device_id, deque(maxlen=MAX_POINTS)).append((float(pos_x), float(pos_y)))
        self._notify(device_id)
